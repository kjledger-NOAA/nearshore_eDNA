...
using this script to calculate amplification efficiencies for each taxa in each mock community. 
...
if com1 is the training data, will the amplification estimate (alpha) be the same regardless of what community is the testing data? i would think so. let me check. - YES THAT"S TRUE

now run each community as the single mock community and have com1 as env 

```{r}
mockcom_data <- mockcom_data %>%
  filter(community %in% c("COM7", "COM12"))
```
...
```{r}
save(Output,file=paste0("/home/kimberly.ledger/nearshore_eDNA/outputs/samples_w_mocks_updated/mock_quant/mock_20241126_in12_out7",".Rdata"))
```

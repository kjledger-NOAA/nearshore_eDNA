...
```{r}
load("/home/kimberly.ledger/nearshore_eDNA/outputs/samples_w_mocks_updated/field_quant/raw_20241122.Rdata")
raw <- Output

load("/home/kimberly.ledger/nearshore_eDNA/outputs/samples_w_mocks_updated/field_quant/mock_20241122.Rdata")
mock1 <- Output 
```
...
```{r}
ggsave(p_clr, filename = "/home/kimberly.ledger/nearshore_eDNA/outputs/samples_w_mocks_updated/figures/clr_all_mocks_combinedestimate.png", width = 8, height = 5, units = "in")
```


import sample metadata 
```{r}
metadata <- read.csv("/home/kimberly.ledger/nearshore_eDNA/data/samples_w_mocks_updated/nearshore_field_metadata.csv") %>%
...
save result.join
```{r}
#write.csv(result.join, "/home/kimberly.ledger/nearshore_eDNA/outputs/samples_w_mocks_updated/field_quant/qm_results_20241125.csv")
```

read in qm results in needed 
```{r}
result.join <- read.csv("/home/kimberly.ledger/nearshore_eDNA/outputs/samples_w_mocks_updated/field_quant/qm_results_20241125.csv") %>%
  select(!X)
```
...
```{r}
biomass <- read.csv("/home/kimberly.ledger/nearshore_eDNA/data/samples_w_mocks_updated/seine_biomass_at_edna_sites.csv") %>%
  select(!X)
...
df3 <- df2 %>%
  ...
  mutate(eDNA = ifelse(raw_mean_of_biological_reps < 0.01, 0, raw_mean_of_biological_reps)) %>%
  mutate(seine = ifelse(is.na(seine), 0, seine))
```

```{r}
fig <- df3 %>%
  ggplot() +
  geom_point(aes(x= seine, y = eDNA, color = SP)) + 
  theme_bw() + 
...
#ggsave(plot = fig, "/home/kimberly.ledger/nearshore_eDNA/outputs/samples_w_mocks_updated/figures/biomass_v_qm.png", dpi = 300, width = 10, height = 6)
ggsave(plot = fig, "/home/kimberly.ledger/nearshore_eDNA/outputs/samples_w_mocks_updated/figures/biomass_v_qm_zoom.png", dpi = 300, width = 10, height = 6)
```

```{r}
fig2 <- df3 %>%
  ggplot() +
  geom_point(aes(x= seine, y = eDNA, color = habitat)) + 
  theme_bw() + 
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black")  +
  facet_wrap(~SP, scales = "free") +
...
ggsave(plot = fig2, "/home/kimberly.ledger/nearshore_eDNA/outputs/samples_w_mocks_updated/figures/biomass_v_qm_by_habitat.png", dpi = 300, width = 10, height = 6)
```

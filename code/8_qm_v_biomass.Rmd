...
calculate mean of qm read proportions per bay
```{r}
df_bay_means <- df_mb %>%
  filter(method == "mock1.mean") %>%
  group_by(year, bay_id, habitat, SP) %>%
  summarize(mean_prop = mean(prop)) %>%
  mutate(method = "qm_mean_of_biological_reps")
```
...
fig <- df3 %>%
  filter(year == 2021) %>%
  ggplot() +
  geom_point(aes(x= seine, y = qm, color = SP)) + 
  theme_bw() + 
...
#ggsave(plot = fig, "outputs/samples_w_mocks_updated/figures/biomass_v_qm.png", dpi = 300, width = 10, height = 6)
```
